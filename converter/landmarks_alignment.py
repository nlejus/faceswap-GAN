...
def get_src_landmarks(x0, x1, y0, y1, pnts):
    ...
    """    
    return [(int(pnts[i+5][0]-x0), int(pnts[i][0]-y0)) for i in range(5)]
...
def get_tar_landmarks(img):
    ...
    img_sz = img.shape
    return [(int(xy[0]*img_sz[0]), int(xy[1]*img_sz[1])) for xy in avg_landmarks]
...
def landmarks_match_mtcnn(src_im, src_landmarks, tar_landmarks):
    ...
    M = umeyama(np.array(src_tmp), np.array(dst_tmp), True)[:2]
    return cv2.warpAffine(
        src_im, M, (src_size[1], src_size[0]), borderMode=cv2.BORDER_REPLICATE
    )